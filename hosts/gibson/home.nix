...
in
{
  ...
  #User shell - START #
	...
	shred = "shred -zfu";
  nixos-rebuild = "systemd-inhibit --no-pager --no-legend --mode block --who='${config.home.username}' --why='NixOS Upgrades' sudo nixos-rebuild --flake ~/nixos-config $@ --option eval-cache false --show-trace";
